...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        logger.info(f"WebSocket connection attempt for conversation {self.conversation_id}")
        ...
        logger.info(f"User {self.user.username} attempting to connect to conversation {self.conversation_id}")

        is_participant = await self.check_conversation_access()
        if not is_participant:
            logger.warning(f"User {self.user.username} denied access to conversation {self.conversation_id}")
            await self.close()
            return

        await self.channel_layer.group_add(self.room_group_name, self.channel_name)
        await self.accept()
        logger.info(f"WebSocket connection established for user {self.user.username} in conversation {self.conversation_id}")

        await self.mark_messages_as_read()

    async def disconnect(self, close_code):
        logger.info(f"WebSocket disconnecting with code {close_code} for conversation {getattr(self, 'conversation_id', 'unknown')}")
